/*---------------------------------------------------------------------------/
/  Petit FatFs - FAT file system module include file  R0.03a
/----------------------------------------------------------------------------/
/ Petit FatFs module is an open source software to implement FAT file system to
/ small embedded systems. This is a free software and is opened for education,
/ research and commercial developments under license policy of following trems.
/
/  Copyright (C) 2019, ChaN, all right reserved.
/
/ * The Petit FatFs module is a free software and there is NO WARRANTY.
/ * No restriction on use. You can use, modify and redistribute it for
/   personal, non-profit or commercial use UNDER YOUR RESPONSIBILITY.
/ * Redistributions of source code must retain the above copyright notice.
/
/----------------------------------------------------------------------------*/
#ifndef PF_DEFINED
#define PF_DEFINED 8088 /* Revision ID */

#include "third_party/pff/pffconf.h"
#include "driver/gpio.h"
#include "system/systick.h"
#include <cstdio>

#if PF_DEFINED != PFCONF_DEF
#error Wrong configuration file (pffconf.h).
#endif

/* Integer types used for FatFs API */
#include <stdint.h>
typedef unsigned int UINT;  /* int must be 16-bit or 32-bit */
typedef unsigned char BYTE; /* char must be 8-bit */
typedef uint16_t WORD;		/* 16-bit unsigned integer */
typedef uint16_t WCHAR;		/* 16-bit unsigned integer */
typedef uint32_t DWORD;		/* 32-bit unsigned integer */

#define PF_INTDEF 1

#if PF_FS_FAT32
#define CLUST DWORD
#else
#define CLUST WORD
#endif

namespace ThirdParty
{
class pff
{
public:
	/* File system object structure */
	struct FATFS
	{
		BYTE fs_type; /* FAT sub type */
		BYTE flag;	/* File status flags */
		BYTE csize;   /* Number of sectors per cluster */
		BYTE pad1;
		WORD n_rootdir;   /* Number of root directory entries (0 on FAT32) */
		CLUST n_fatent;   /* Number of FAT entries (= number of clusters + 2) */
		DWORD fatbase;	/* FAT start sector */
		DWORD dirbase;	/* Root directory start sector (Cluster# on FAT32) */
		DWORD database;   /* Data start sector */
		DWORD fptr;		  /* File R/W pointer */
		DWORD fsize;	  /* File size */
		CLUST org_clust;  /* File start cluster */
		CLUST curr_clust; /* File current cluster */
		DWORD dsect;	  /* File current data sector */
	};

	/* Directory object structure */
	struct DIR
	{
		WORD index;   /* Current read/write index number */
		BYTE *fn;	 /* Pointer to the SFN (in/out) {file[8],ext[3],status[1]} */
		CLUST sclust; /* Table start cluster (0:Static table) */
		CLUST clust;  /* Current cluster */
		DWORD sect;   /* Current sector */
	};

	/* File status structure */
	struct FILINFO
	{
		DWORD fsize;	/* File size */
		WORD fdate;		/* Last modified date */
		WORD ftime;		/* Last modified time */
		BYTE fattrib;   /* Attribute */
		char fname[13]; /* File name */
	};

	/* File function return code (FRESULT) */
	enum FRESULT
	{
		FR_OK = 0,		 /* 0 */
		FR_DISK_ERR,	 /* 1 */
		FR_NOT_READY,	/* 2 */
		FR_NO_FILE,		 /* 3 */
		FR_NOT_OPENED,   /* 4 */
		FR_NOT_ENABLED,  /* 5 */
		FR_NO_FILESYSTEM /* 6 */
	};

	pff();

	/*--------------------------------------------------------------*/
	/* Petit FatFs module application interface                     */
	FRESULT pf_mount(FATFS *fs);							/* Mount/Unmount a logical drive */
	FRESULT pf_open(const char *path);						/* Open a file */
	FRESULT pf_read(void *buff, UINT btr, UINT *br);		/* Read data from the open file */
	FRESULT pf_write(const void *buff, UINT btw, UINT *bw); /* Write data to the open file */
	FRESULT pf_lseek(DWORD ofs);							/* Move file pointer of the open file */
	FRESULT pf_opendir(DIR *dj, const char *path);			/* Open a directory */
	FRESULT pf_readdir(DIR *dj, FILINFO *fno);				/* Read a directory item from the open directory */

private:
	/* Results of Disk Functions */
	enum DRESULT
	{
		RES_OK = 0, /* 0: Function succeeded */
		RES_ERROR,  /* 1: Disk error */
		RES_NOTRDY, /* 2: Not ready */
		RES_PARERR  /* 3: Invalid parameter */
	};
	typedef BYTE DSTATUS; /* Status of Disk Functions */

	FATFS *FatFs; /* Pointer to the file system object (logical drive) */
	Driver::GPIO spi, spo, clk, cs;
	int csToSckDelay, sckDelay; // boot speed should be 100khz > & < 400khz
	BYTE CardType;				/* Detected card type */

	WORD ld_word(const BYTE *ptr);
	DWORD ld_dword(const BYTE *ptr);
	void mem_set(void *dst, int val, int cnt);
	int mem_cmp(const void *dst, const void *src, int cnt);
	CLUST get_fat(CLUST clst);
	DWORD clust2sect(CLUST clst);
	CLUST get_clust(BYTE *dir);
	FRESULT dir_rewind(DIR *dj);
	FRESULT dir_next(DIR *dj);
	FRESULT dir_find(DIR *dj, BYTE *dir);
	FRESULT dir_read(DIR *dj, BYTE *dir);
	FRESULT create_name(DIR *dj, const char **path);
	void get_fileinfo(DIR *dj, BYTE *dir, FILINFO *fno);
	FRESULT follow_path(DIR *dj, BYTE *dir, const char *path);
	BYTE check_fs(BYTE *buf, DWORD sect);

	/*---------------------------------------*/
	/* Prototypes for disk control functions */
	DSTATUS disk_initialize(void);
	DRESULT disk_readp(BYTE *buff, DWORD sector, UINT offser, UINT count);
	DRESULT disk_writep(const BYTE *buff, DWORD sc);

	/*---------------------------------------*/
	/* soft spi init                         */
	/* function declaration from pic24_mmcp.c */
	const Driver::GPIO::Config getGpioConfig(System::Pinout::Name pinName, bool isOutput, bool isPullup);
	inline void xmit_spi(BYTE d)
	{
		// spi mode 0
		// shift data
		spo.Set(d & 0x80);
		System::Systick::DelayNS(sckDelay);

		for (int i = 0; i < 7; ++i)
		{
			// latch data
			clk.Set(true);
			d <<= 1;
			System::Systick::DelayNS(sckDelay);

			// shift data
			clk.Set(false);
			spo.Set(d & 0x80);
			System::Systick::DelayNS(sckDelay);
		}

		// latch data
		clk.Set(true);
		// d << 1;
		System::Systick::DelayNS(sckDelay);

		// shift data
		clk.Set(false);
		// spo.Set(d & 0x80);
		System::Systick::DelayNS(sckDelay);
	}
	inline BYTE rcv_spi()
	{
		BYTE dat = 0;
		// spi mode 0

		for (int i = 0; i < 7; ++i)
		{
			// latch data
			clk.Set(true);
			dat |= spi.Get();
			System::Systick::DelayNS(sckDelay);

			// shift data
			clk.Set(false);
			dat <<= 1;
			System::Systick::DelayNS(sckDelay);
		}

		// latch data
		clk.Set(true);
		dat |= spi.Get();
		System::Systick::DelayNS(sckDelay);

		// shift data
		clk.Set(false);
		// dat <<= 1; no need shift at last
		System::Systick::DelayNS(sckDelay);

		return dat;
	}
	inline void CS_HIGH()
	{
		cs.Set(true);
	}
	inline void CS_LOW()
	{
		cs.Set(false);
	}
	inline void dly_100us()
	{
		System::Systick::DelayUS(100);
	}
	BYTE send_cmd(BYTE cmd, DWORD arg);

	void put_rc(FRESULT rc)
	{
		switch (rc)
		{
		case FRESULT::FR_OK:
			printf("FR_OK\n");
			break;
		case FRESULT::FR_DISK_ERR:
			printf("FR_DISK_ERR\n");
			break;
		case FRESULT::FR_NOT_READY:
			printf("FR_NOT_READY\n");
			break;
		case FRESULT::FR_NO_FILE:
			printf("FR_NO_FILE\n");
			break;
		case FRESULT::FR_NOT_OPENED:
			printf("FR_NOT_OPENED\n");
			break;
		case FRESULT::FR_NOT_ENABLED:
			printf("FR_NOT_ENABLED\n");
			break;
		case FRESULT::FR_NO_FILESYSTEM:
			printf("FR_NO_FILESYSTEM\n");
			break;
		}
	}
	void put_drc(BYTE res)
	{
		char t[10];
		sprintf(t, "rc=%d\n", res);
		printf(t);
	}
};
};	 // namespace ThirdParty
#endif /* _PFATFS */
