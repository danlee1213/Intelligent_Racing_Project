...
MPU6050::MPU6050(Config &config) : temp(0.0f), is_calibrated(false), gyro_range(config.gyro_range), accel_range(config.accel_range), gyro_scale_factor(GetGyroScaleFactor()), accel_scale_factor(GetAccelScaleFactor())
{
	...
	else
	{
		...
		scl_config.pin = MPU6050_SCK(config.id);
		sda_config.pin = MPU6050_SDA(config.id);
		Driver::I2CMaster::Config i2c_config = MPUGetI2CConfig(scl_config.pin, sda_config.pin);
...
bool MPU6050::Verify()
{
	...
	{
		return (who_am_i == 0x68)||(who_am_i == 0x72);
	}
}
