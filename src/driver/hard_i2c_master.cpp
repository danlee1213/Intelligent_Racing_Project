...
HardI2CMaster::HardI2CMaster(const I2CMaster::Config &config) : send_wait_time(config.send_wait_time), recieve_wait_time(config.recieve_wait_time), scl(config.scl), sda(config.sda)
{
...
{
	uint32_t start_wait_time = Systick::GetTimeInUS();
	while (1)
	{
		...
		if (tx_count)
			return true;
		if ((Systick::GetTimeInUS() - start_wait_time) < send_wait_time)
		{
			return false;
		}
	}
...
bool HardI2CMaster::GetByte(uint8_t &byte)
{
	...
	while (1)
	{
		...
		if (Systick::GetTimeInUS() - start_wait_time > recieve_wait_time)
		{
			return false;
		}
	}
...
bool HardI2CMaster::GetByte(uint8_t *byte, bool /* last_byte */) {
	// last byte setting ignored !!!
	// if u need the function plz add it
	return GetByte(*byte);	
}
